    ...
    <form  className='min-h-[80vh] flex items-center'>
      ...
      <div className='flex flex-col gap-3 m-auto items-start p-8 min-w-[340px] sm:min-w-96 border rounded-xl text-zinc-600 text-sm shadow-lg'>
        ...
        <p>Please {state == 'Sign Up' ? "sign up" :"login" } to book appointment</p>
        {
          ...
          {
            state === "Sign Up"
            ? <p>Already have an account? <span onClick={()=>setState('Login')} className='text-primary underline cursor-pointer'>Login here</span> Login here</p>
            : <p>Create an new account? <span onClick={()=>setState('Sign Up')}  className='text-primary underline cursor-pointer'>click here</span> click here</p>
          }
      </div>
